...
| S-01 | ai-generation-flow    | wkleić tekst → AI generuje kandydatów → recenzja → zaakceptowane w kolekcji | F-01         | FR-004, FR-005, FR-006, US-01  | blocked  |
...
- **Blockers:** —
- **Unknowns:**
  - Który dostawca LLM (OpenAI / Anthropic / OpenRouter / inny)? — Owner: user. Block: yes.
...
- **Risk:** jakość kandydatów AI to serce propozycji wartości — jeśli prompt daje systematycznie słabe wyniki, cały MVP traci rację bytu; iteracja na prompcie jest wbudowana w implementację, ale wybór dostawcy LLM musi być rozstrzygnięty przed jakimkolwiek kodem integracji
- **Status:** blocked
...
| S-01       | ai-generation-flow    | Feature: AI generation flow (wklej → kandydaci → kolekcja)      | no                    | Zablokowane — zdecyduj najpierw o dostawcy LLM  |
...
1. **Wybór dostawcy LLM (OpenAI / Anthropic / OpenRouter / inny)** — Owner: user. Block: S-01 (bez tej decyzji S-01 nie nadaje się do planowania).
...
- **Własny algorytm spaced-repetition (SuperMemo / Anki scheduler)** — PRD §Non-Goals: budowanie własnego SRS = miesiące pracy; Anki obsługuje powtórki przez eksport CSV.
...
- **Własne UI nauki + własny scheduler powtórek** — wyłonione w shaping: Anki obsługuje SRS po imporcie CSV.
